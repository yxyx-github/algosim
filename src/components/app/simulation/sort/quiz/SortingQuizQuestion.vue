<template>
    <FColumn v-if="![question.algorithm, question.simulation].includes(undefined)">
        ...
        <SimulationView :simulation="question.simulation as SortSimulation">
...
const solutionAlgorithmLabel = computed(() => sortAlgorithmData.filter(algorithm => algorithm.value === question.algorithm).map(algorithm => algorithm.label)[0] ?? '')
